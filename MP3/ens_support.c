...
epid_rec_t *make_rec(int id);
void ens_appexp(linked_t *list_ptr, int id);
epid_rec_t *make_rec(int id);




void ens_list_sort(linked_t *list_ptr, int sort_type)
{
    clock_t start, end;
    double elapse_time; //time in milliseconds


    int initialcount = linked_entries(list_ptr);
    start = clock();
    linked_sort(list_ptr, sort_type, ens_compare_epid);
    end = clock();
    elapse_time = 1000.0 * ((double) (end - start)) / CLOCKS_PER_SEC;
    assert(linked_entries(list_ptr) == initialcount);
...
/*
    Purpose: Creates a new node and appends the node to the back of the unsorted "exposure" list
    
    Input: 
        -list_ptr: pointer to the unsorted list
        -id: id to be assigned to the new node
*/
void ens_appexp(linked_t *list_ptr, const int id)
{    
    epid_rec_t *new_ptr = make_rec(id);    
    linked_insert(list_ptr, new_ptr, LLPOSITION_BACK);
}


/*
    Purpose: creates the new node and assigns values to its data
...
epid_rec_t *make_rec(int id)
{
    ...
    new_rec = (epid_rec_t *)calloc(1, sizeof(epid_rec_t));
    assert(new_rec != NULL);
    seq_num++;
    
    new_rec -> ep_id = id;
    new_rec -> rec_num = seq_num;
    return new_rec;
}

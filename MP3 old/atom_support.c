...
/* Adds a new node to the tail BUT has a value of 0 for all data values except atom id
 * Input:
 * 	list_ptr: Unsorted list ptr
 * 	atom_id: Atom id of the new tail node
 * Output:N/A
*/
void atom_list_add_tail_mp3(list_t *list_ptr, int atom_id)
{
	atom_t *rec_ptr = (atom_t*)calloc(1, sizeof(rec_ptr));
	rec_ptr -> atom_id = atom_id;
 ...
 *  Input:
 *  	list_ptr: Unsorted list ptr
 *  	sort_type: which type of sort algo to use
 *  		1: bubble sort
 *  		2: insertion sort
...
void atom_list_print(list_t * list_ptr, char *list_type)
{
	...
	atom_t *rec_ptr = list_access(list_ptr, LISTPOS_HEAD);
...
//If list isn't empty, iterate thru the list and print each node's info
	else 
	{
...
void atom_list_stats(list_t * sorted, list_t * unsorted, list_t* mp3)
{
	...
	printf("Number records: %d, Order: %s\n", num_in_sorted_list, order_of_list == 1 ? "Ascending" : "Descending");
...
{
//Initializes and allocates rec_ptr
	...
	added_return = list_size(list_ptr) >= max_list_size ? -1:1;
...
//Iterates through the list. If a node that matches e_pot, the loop is broken and the node position is saved in "i"
...
//If e_pot match not found, print not found. If e_pot is found, then print the node stats
	if (rec_ptr == NULL) 
	{
...
//If e_pot not found, print not found. If e_pot is found, remove the node from the list and free rec_ptr
	if (rec_ptr == NULL) 
	{
	...
	{
		rec_ptr = list_remove(list_ptr, i);
	        printf("Removed: %e\n", e_pot);
	        print_atom_rec_long(rec_ptr);
		rec_ptr = NULL;
    	}
...
//Creates a new list for the atoms found outside the box and creates the bounding box
	list_t* migrate_list = list_construct(atom_compare, atom_rec_cleanup);
 	get_bounding_box(&minX, &maxX, &minY, &maxY); 



//Builds new migrate list
	for (i = 0; i < list_size(list_ptr); i++)
	{
		rec_ptr = list_access(list_ptr, i);


	//Checks to see if each node is inside or outside the bounding box. If the atom is found outside, then transfer it from the main list into the migrate list
		if (determine_inside_box(rec_ptr, minX, maxX, minY, maxY) == 0)
		{
			list_insert_sorted(migrate_list, rec_ptr);
			list_remove(list_ptr, i);
		}
	}
	...
	//Iterates thru the migrate list and prints each atom's info
        	for (i=0; i < count; i++) 
		{
...
//i for loop handles iterating thru each node. j for loop handles iterating thru each thing to update
	for (i = 0; i < list_ptr -> current_list_size - 1; i++)
	{
		...
		//advance forces
		rec_ptr -> potential_energy = (float)drand48();
		for (j = 0; j <= 1; j++)
		{
			rec_ptr -> force[j] = (float)drand48();
		}
		...
		for (j = 0; j <= 1; j++)
		{
			rec_ptr -> momenta[j] = (rec_ptr -> momenta[j])*(rec_ptr -> position[j]);
		}
		...
		for (j = 0; j <= 1; j++)
		{
			rec_ptr -> position[j] = (rec_ptr -> position[j])+((dt*rec_ptr -> momenta[j])/(rec_ptr -> mass));
		}
	}
}
                        ...
                        float x_max, float y_min, float y_max)
{
	...
	float x = rec_ptr -> position[0];
	float y = rec_ptr -> position[1];


//Checks to see if node/atom is within the bounding box, including right on the line
	if (x >= x_min && x <= x_max)
	{
		if (y >= y_min && y <= y_max)
		{
...
{
	//Inserts a node at the tail and marks the list used as unsorted
	atom_t *rec_ptr = (atom_t*)malloc(sizeof(atom_t));
	fill_atom_record(rec_ptr);
	list_insert(list_ptr, rec_ptr, LISTPOS_TAIL);
...
{
	atom_t *atom_ptr = list_remove(list_ptr, LISTPOS_HEAD);
...
{
	atom_t *rec_ptr = list_access(list_ptr, 0);

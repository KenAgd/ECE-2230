...
//Tests blocks almost the size of a page
	...
	//Allocation of all 3 blocks
		num_bytes_1 = PAGESIZE; //control block
		p1 = Mem_alloc(num_bytes_1 - unit_size);
		printf("first: %d bytes (%d units) p=%p \n", num_bytes_1, num_bytes_1/unit_size, p1);


		num_bytes_2 = PAGESIZE + 1; //block is 1 size above normal page size
		p2 = Mem_alloc(num_bytes_2 - unit_size);
		 printf("second: %d bytes (%d units) p=%p \n", num_bytes_2, num_bytes_2/unit_size, p2);


		num_bytes_3 = PAGESIZE - 1; // block is 1 size below normal page size
		p3 = Mem_alloc(num_bytes_3 - unit_size);
	...
	//Frees all 3 blocks one at a time
		Mem_free(p1);
		p1 = NULL;
...
//Tests coalescing   
	...
	//allocs and frees blocks 1 and 2 five times
		for (i = 0; i < 5; i++)
		{
			p1 = Mem_alloc(num_bytes_1 - unit_size);
			p2 = Mem_alloc(num_bytes_2 - unit_size);
...
void equilibriumDriver(driver_params *ep)
{
    ...
    mem_list = linked_construct(NULL);
    ...
    for (i = 0; i < ep->WarmUp; i++) {
        ...
        linked_insert(mem_list, (data_t *) ptr, LLPOSITION_BACK);
        ptr = NULL;
    }
    printf("After warmup\n");
    if (!ep->SysMalloc) {
    ...
    for (i = 0; i < ep->Trials; i++) {
        if (drand48() < 0.5) {
            ...
            linked_insert(mem_list, (data_t *) ptr, LLPOSITION_BACK);
            ptr = NULL;
        } else if (linked_entries(mem_list) > 0) {
            pos = (int) (drand48() * linked_entries(mem_list));
            ptr = (int *) linked_remove(mem_list, pos);
            assert(ptr != NULL);
            size = -ptr[0];
            if (ep->Verbose) {
                // uncomment following print for more detail
    ...
    pos = linked_entries(mem_list);
    for (i = 0; i < pos; i++) {
        ptr = (int *) linked_remove(mem_list, LLPOSITION_FRONT);
        assert(ptr != NULL);
        size = -ptr[0];
        assert(min_num_ints <= size && size <= ep->AvgNumInts+ep->RangeInts);
        for (index = 1; index < size; index++)
    ...
    assert(linked_entries(mem_list) == 0);
    linked_destruct(mem_list);

...
static int Seed = 11302021;
static int UnitDriver = -1;
...
int main(int argc, char **argv)
{
    ...
    if (UnitDriver == 1)
    {
        printf("--Begin unit driver 1--\n");
    //first create the table and array indices, then fill the 8 values
        T = table_construct(TableSize, ProbeDec);
        ...
        printf("\nInserting 8 keys\n");
    //add all 8 indices into the table and print table (if verbose enabled)
        table_insert(T, 110, &(i[0]));
        table_insert(T, 220, &(i[1]));
        table_insert(T, 330, &(i[2]));
        table_insert(T, 440, &(i[3]));
        table_insert(T, 550, &(i[4]));
        table_insert(T, 660, &(i[5]));
        table_insert(T, 770, &(i[6]));
        table_insert(T, 880, &(i[7]));
    ...
    //used asserts to double check all entries were inserted and in the right spot   	
        assert(table_entries(T) == 8);
        assert(table_retrieve(T, 110) == &(i[0]));
        assert(table_retrieve(T, 220) == &(i[1]));
        assert(table_retrieve(T, 330) == &(i[2]));
        assert(table_retrieve(T, 440) == &(i[3]));
        assert(table_retrieve(T, 550) == &(i[4]));
        assert(table_retrieve(T, 660) == &(i[5]));
        assert(table_retrieve(T, 770) == &(i[6]));
        assert(table_retrieve(T, 880) == &(i[7]));
    
    	
        printf("\nRemoving 8 keys\n");
    //remove all keys from table and print table (if verbose enabled)
        free(table_delete(T, 110));       
        table_delete(T, 220);
        table_delete(T, 330);
        table_delete(T, 440);
        table_delete(T, 550);
    ...
    //assert to double check list is empty and then destruct table    	        
        assert(table_entries(T) == 0);
    ...
    if (UnitDriver == 2)
    {
        printf("----- Begin unit driver 2 -----\n");
	
	T = table_construct(6, DOUBLE);
	...
	printf("Inserting 4 keys\n");
	table_insert(T, 25, &(i[0]));
	table_insert(T, 30, &(i[1]));
	table_insert(T, 35, &(i[2]));
	printf("this should print before a crash\n");
	
	if (Verbose)
	{
	    table_debug_print(T);
	}
	//should crash right here upon insertion
	table_insert(T, 40, &(i[3]));
...
//deletion driver
	...
	table_insert(T, 5, malloc(sizeof(int)));
	table_insert(T, 12, malloc(sizeof(int)));
	table_insert(T, 11, malloc(sizeof(int)));
	table_insert(T, 19, malloc(sizeof(int)));
	table_debug_print(T);
	
	printf("\nRemove keys 5 8 12\n");
	free(table_delete(T, 5));
	free(table_delete(T, 8));
        free(table_delete(T, 12));
	table_debug_print(T);
	
	printf("\nInsert keys 19 26\n");
	table_insert(T, 19, malloc(sizeof(int)));
	table_insert(T, 26, malloc(sizeof(int)));
	table_debug_print(T);
	...
	table_insert(T, 7, malloc(sizeof(int)));
	table_insert(T, 8, malloc(sizeof(int)));
	table_insert(T, 9, malloc(sizeof(int)));
	table_insert(T, 10, malloc(sizeof(int)));
	table_insert(T, 11, malloc(sizeof(int)));
	table_insert(T, 12, malloc(sizeof(int)));
	table_debug_print(T);

	printf("\nRemove 7 8 9 10\n");
	free(table_delete(T, 7));
	free(table_delete(T, 8));
	free(table_delete(T, 9));
	free(table_delete(T, 10));
	table_debug_print(T);

	printf("\nInsert 13 14\n");
	table_insert(T, 13, malloc(sizeof(int)));
	table_insert(T, 14, malloc(sizeof(int)));
	table_debug_print(T);

	printf("\nRetrieve 16\n");
	assert(table_retrieve(T, 16) == NULL);
	printf("16 not found.\n");
...
void getCommandLine(int argc, char **argv)
{
    ...
    while ((c = getopt(argc, argv, "m:a:h:i:t:s:u:erbv")) != -1)
        switch(c) {
            ...
            case 'u': UnitDriver = atoi(optarg);     break;
            case 'h':

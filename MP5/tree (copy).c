...
//int tree_debug_validate_rec(bst_node_t *N, int min, int max, int *count);
...
//Recursively go thru and destruct the tree, going thru one by one clearing each node's data and their connections
 ...
 */
int tree_debug_validate_rec(tree_node_t *N, int min, int max, int *count)
{
    if (N == NULL)
        return TRUE;
    ...
    *count += 1;
    return tree_debug_validate_rec(N->left, min, N->key, count) &&
        tree_debug_validate_rec(N->right, N->key, max, count);
}
...
{
    printf("Tree with %d keys\n", T -> count); //look at given pdf, dont know what ItemCount is, just left as count for now
    ugly_print(T->root, 0);
    printf("\n");

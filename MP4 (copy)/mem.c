...
//    assert(p->size_units > 1);   // verify that the size field is valid
	...
	mem_chunk_t *free_ptr = return_ptr;
	free_ptr--;


	if (Rover + Rover -> size_units == free_ptr)
	{
		pre = 1;
	}


	if (free_ptr + free_ptr -> size_units == free_ptr -> next)
	{
		post = 1;
	}
	...
	if (pre && post)
	{
		Rover -> size_units += free_ptr -> size_units;
		Rover -> next = free_ptr -> next;
		free_ptr -> next = NULL;
	...
	{
		free_ptr -> size_units += free_ptr -> next -> size_units;
